...
- [R] **CRM-13** → `specs/retroactive-sale-date-and-deletion` — Data da venda escolhível (campo pré-preenchido com hoje, alterável para qualquer data passada) e exclusão de venda pela consultora (reversão de estoque + remoção, com confirmação em dois passos) — _dep: CRM-12_ — handoff 2026-09-19; QA aprovada na rodada 4 (1.317 testes, lint, typecheck, build e cadeia de migrações 0010→0014 verdes). Aguardando commit humano.
- [ ] **CRM-14** — Leitura de período passado: seletor de mês no dashboard (faturamento e lucro de qualquer mês, não só o corrente) e filtro de data na listagem de vendas — _dep: CRM-13_ — achado da revisão neutra do CRM-13 em 2026-09-19: aquela feature torna o **dado** correto, mas hoje `monthSalesScope` agrega só o mês corrente e `salesListQuerySchema` não tem filtro de data — o histórico reinserido fica certo e ilegível por período. Decisão do humano em 2026-09-19: item separado, para não segurar a base
